...
SELECT name,salary,hire_date,LAG(hire_date,1)OVER(ORDER BY hire_date) FROM employ;

SELECT name,salary,hire_date,LEAD(hire_date,1)OVER(ORDER BY hire_date) FROM employ;


SELECT * FROM employ ORDER BY department;
...
SELECT 
	id,department,salary,
	AVG(salary) OVER (PARTITION BY department) AS avg_dept_salary,
    ROW_NUMBER() OVER (PARTITION BY department) AS row_dept_salary,
	RANK() OVER (PARTITION BY department ORDER BY salary DESC) AS rank,
	DENSE_RANK() OVER (PARTITION BY department ORDER BY salary DESC) AS dense_rank,
